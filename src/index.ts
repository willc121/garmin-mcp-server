...
const SUPABASE_URL =
  process.env.SUPABASE_URL || "https://ylcbldppuaugitdnrjxv.supabase.co";
...
async function getActivities(
  ...
  let filtered = all;
  if (startDate) filtered = filtered.filter((a) => a.start_time >= startDate);
  // Make endDate exclusive to match your schemas
  if (endDate) filtered = filtered.filter((a) => a.start_time < endDate);
  if (activityType)
    filtered = filtered.filter((a) => a.activity_type === activityType);

  const groups: Record<
    string,
    { count: number; totalKm: number; totalHours: number }
  > = {};

  filtered.forEach((a) => {
    const type = a.activity_type || "unknown";
    if (!groups[type]) groups[type] = { count: 0, totalKm: 0, totalHours: 0 };
    groups[type].count += 1;
...
const server = new Server(
  { name: "garmin-health-mcp-server", version: "1.0.0" },
  { capabilities: { tools: {} } }
);

server.setRequestHandler(ListToolsRequestSchema, async () => {
  // Single source of truth: tools.catalog.ts
  return { tools: TOOL_CATALOG as any };
});
